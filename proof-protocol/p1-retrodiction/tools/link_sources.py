...
T = [
 ...
 (f"{P}/cases/01-sensitivity/before.json", "sources_before[1]", "Gotsman", "C1-gotsman", True, PRIMARY,
  "CLOSED from the Huang transcript, which states the Gotsman-Linial equivalence as its Theorem 1.3 and gives the publication data (JCTA 61(1) 1992, 142-146). It was the last UNVERIFIED row in the tree and the only one no pass had ever checked; D19 closed with it. Note the source's form: the equivalence is about Gamma(H) = max{Delta(H), Delta(Q_n - H)} under |V(H)| != 2^{n-1}, for a general monotone h -- the 'more than half' step is Huang's bridge to h = sqrt(n), not part of the equivalence."),
 ...
 (f"{P}/twins/N3-3ap-free-integers.json", "verification_sources[0]", "Peluse", "N3", True, PRIMARY,
  "The transcript itself."),
 (f"{P}/twins/N9-huang-tightness.json", "sources[0]", "Chung", "C1-bound", True, PRIMARY,
  "Row C1-bound is now primary-tier: the Huang transcript states CFGS's (1/2 - o(1)) log_2 n AND their matching (2^{n-1}+1)-vertex construction of maximum degree ceil(sqrt n), which is the extremal input this twin is about. Row C1-tightness adds what the paper actually claims -- Delta tight at PERFECT SQUARES, lambda_1 best possible for ALL n -- and records that the audit's all-n strengthening is ours, not the paper's. The N9 audit broke this twin's ROLE, not its citation."),
